...
//			long catchtime = Long.parseLong(data);
//			if((time - catchtime) > (1000*60*24*30*3)){
//				getDataFromNet();
//			}else{
//				list = JsonUtil.getSchedules(data);
//				initView(list);
...
//以下内容除了setCourse3 暂时没用，逻辑有点问题
	...
	private void getDataFromNet(){
		...
		try {
			json.put("username","13223227");
			json.put("password","gzw19920205");
		} catch (JSONException e) {
			e.printStackTrace();
		}
		gn.getSchedule(Config.URL, Config.SCHEDULE, new Response.Listener<String>() {
			@Override
			public void onResponse(String s) {
				Config.saveSchedule(ScheduleAty.this,s);
				list = JsonUtil.getSchedules(s);
				initView(list);
			}
			...
			public void onErrorResponse(VolleyError volleyError) {
				ShowToast.show(ScheduleAty.this,"获取数据失败");
			}
		},json.toString());
	}
